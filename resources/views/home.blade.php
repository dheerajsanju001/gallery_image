...
<div class="flex mx-auto items-center justify-center shadow-lg  mx-8 mb-4 max-w-lg">
    <form class="w-full max-w-xl bg-white rounded-lg px-4 pt-2" action="comment_form{{ $d->id }}" method="post">
        {{ csrf_field() }}
       <div class="flex flex-wrap -mx-3 mb-6">
          <h2 class="px-4 pt-3 pb-2 text-gray-800 text-lg">Add a new comment</h2>
          <div class="w-full md:w-full px-3 mb-2 mt-2">
            <input class="bg-blue" type="hidden" value="{{ session('signup_id') }}" name="sign_id">
                     <input class="bg-blue" type="hidden" value = "{{ $d->id}}"name="pic_id" placeholder="image">
             <textarea class="bg-gray-100 rounded border border-gray-400 
          ...
          <div class="w-full md:w-full flex items-start md:w-full px-3">
             ...
             <div class="-mr-1">
                ...
                <input type='submit' class="bg-white text-gray-700 font-medium py-1 px-4 border border-gray-400 rounded-lg
                 tracking-wide mr-1 hover:bg-gray-100" name="submit" value='Post Comment'>
                 @else
                 <a href="login"><input type='button' class="bg-white text-gray-700 font-medium py-1 px-4 border border-gray-400 rounded-lg
                 tracking-wide mr-1 hover:bg-gray-100" name="submit" value='Login first'></a>
              @endif
             </div>
          </div>
       </form>
